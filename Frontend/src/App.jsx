...
export default function ChatApp({ user }) {
  ...
  useEffect(() => {
    ...
    const handleMessage = (msg) => {
      console.log("📩 Received message from server:", msg); // 🔹 Debugging Added
      setMessages((prev) => [...prev, msg]);
    };
  ...
  const selectUser = async (selectedUsername) => {
    setSelectedUser(selectedUsername);
    setMessages([]); // 🔹 Fix: setMessage([]) ki jagah setMessages([])
    const chatRoom = [user, selectedUsername].sort().join("-");
    setRoom(chatRoom);

    console.log("🔗 Joining room:", chatRoom); // 🔹 Debugging Added
  ...
  useEffect(() => {
    if (room) {
      ...
      console.log("🚀 Joined room:", room); // 🔹 Debugging Added
    }
    return () => {
      socket.emit("leave_chat", { room });
      console.log("🚪 Left room:", room); // 🔹 Debugging Added
    };
  }, [room]);
